...
Suit Card::get_suit(Suit trump) const{
  Suit trump_next = Suit_next(trump);
  if(rank==JACK&&suit==trump_next) return trump;
  return suit;
}
...
bool Card::is_left_bower(Suit trump) const{
  Suit trump_next = Suit_next(trump);
  return (rank==JACK&&suit==trump_next);
}

//EFFECTS Returns true if the card is a trump card.  All cards of the trump
// suit are trump cards.  The left bower is also a trump card.
bool Card::is_trump(Suit trump) const{
  return (is_left_bower(trump)||suit==trump); //Requires further inspection
}
...
bool Card_less(const Card &a, const Card &b, const Card &led_card, Suit trump){
  ...
  // When none of the cards are led cards
  if(a.get_suit()!=led_suit&&b.get_suit()!=led_suit) return Card_less(a, b, trump);

  // Consider Trump vs Not Trump
  if(a.is_trump(trump)&&!b.is_trump(trump)) return false;
  if(!a.is_trump(trump)&&b.is_trump(trump)) return true;
